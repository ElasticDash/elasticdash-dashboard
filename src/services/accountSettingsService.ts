...
export async function fetchOauthToken() {
	const res = await axios.get(process.env.NEXT_PUBLIC_BASE_URL + '/user/settings/oauthtoken', {
		headers: {
			Authorization: `Bearer ${localStorage.getItem('token') || ''}`
		}
	});
...
export async function updateOauthToken(oauthToken: string) {
	const res = await axios.put(
		process.env.NEXT_PUBLIC_BASE_URL + '/user/settings/oauthtoken',
		{ oauthToken },
		{
			headers: {
				'Content-Type': 'application/json',
